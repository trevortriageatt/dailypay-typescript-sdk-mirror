 ...
 */
export function paychecksRead(
  client: SDKCore,
  request: operations.ReadPaycheckRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ReadPaycheckResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ReadPaycheckRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    paycheck_id: encodeSimple("paycheck_id", payload.paycheck_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/rest/paychecks/{paycheck_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ReadPaycheckResponse$inboundSchema, {
