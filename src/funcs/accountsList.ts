...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAccountsRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    ...
    "filter[account_type]": payload?.["filter[account_type]"],
    "filter[person.id]": payload?.["filter[person.id]"],
    "filter[subtype]": payload?.["filter[subtype]"],
  });
