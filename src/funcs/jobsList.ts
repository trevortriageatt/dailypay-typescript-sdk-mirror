...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListJobsRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/rest/jobs")();

  const query = encodeFormQuery({
    "filter": payload?.["filter-by"],
    "filter[external_identifiers.employee_id]": payload
      ?.["filter[external_identifiers.employee_id]"],
    "filter[external_identifiers.group]": payload
      ?.["filter[external_identifiers.group]"],
    "filter[external_identifiers.primary_identifier]": payload
      ?.["filter[external_identifiers.primary_identifier]"],
    "filter[organization.id]": payload?.["filter[organization.id]"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListJobsResponse$inboundSchema, {
