...
import { SDKDefaultError } from "../models/errors/sdkdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SDKDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SDKDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SDKDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
